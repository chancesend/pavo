...
abstract class Pattern extends LXPattern implements DeckControllerListener {

  protected final DeckController deckController;

  protected ColorTrackerSet colorTrackerSet;
  protected ColorPaletteIterator colorPaletteIterator;
  ...
  final DiscreteParameter setting = new DiscreteParameter("Setting", 19);

  public Pattern(LX lx, DeckController deckController) {
    super(lx);
    this.deckController = deckController;
    this.model = (Model)super.model;
    this.section = deckController.section;
    this.leds = section.leds;
    this.isInside = deckController.isInside;
    this.isOutside = deckController.isOutside;
    this.inside = deckController.inside;
    this.outside = deckController.outside;
    this.feathers = deckController.feathers;
    this.ceiling = deckController.ceiling;
    this.benches = deckController.benches;

    deckController.addListener(this);
    colorPaletteIterator = deckController.getCurrentPalette().getIterator();
    colorPaletteIterator.next();

    colorTrackerSet = new ColorTrackerSet(deckController);

    setting.addListener(new LXParameterListener() {
      public void onParameterChanged(LXParameter parameter) {
        onSettingChange(setting.getValuei());
      }
    });
  }
  ...
  void resetPalette() {
    colorPaletteIterator.reset();
  }

  void setPaletteStartRandom() {
    colorPaletteIterator.setStartRandom();
  }

  boolean hasNextColor() {
    return colorPaletteIterator.hasNext();
  }

  int getNextColor() {
    return colorPaletteIterator.next();
  }

  int getCurrentColor() {
    return colorPaletteIterator.getCurrent();
  }

  ColorTracker getNewColorTracker() {
    return colorTrackerSet.getNewColorTracker();
  }

  public void onActivePaletteChanged(ColorPalette newPalette) {
    colorPaletteIterator = newPalette.getIterator();
  }

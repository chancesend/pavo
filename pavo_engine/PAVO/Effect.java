...
abstract class Effect extends LXEffect implements DeckControllerListener {
  ...
  final BasicParameter strength = new BasicParameter("VEL", 1);

  public Effect(LX lx, DeckController deckController) {
    super(lx);
    ...
    this.outside = model.outside;
    this.feathers = model.feathers;
    this.ceiling = model.ceiling;
    this.benches = model.benches;
  ...
  public void loop(double deltaMs) {
    if (!enabled.isOn() || strength.getValue() == 0) return;
    super.loop(deltaMs);
  }
